...
| FR-2a | On the first interactive run with no `agent.yaml`, the CLI MUST offer a setup wizard (host → tool-capable model → reasoning visibility) and write the config, then continue. `--setup` MUST force it even when a config exists, seeding prompts with current values. It MUST be skipped when `--model`/`--host` are given (the user is already steering) and in non-interactive/`--print` runs (which fall back to defaults). Cancelling (Ctrl-C) MUST proceed on defaults/existing config, not abort. | Should |
...
|---|-----------------|-------|------------|
| 1 | Small local models (~8B) tool-call unreliably. | Claude | **Measured at the M2 checkpoint, 2026-07-24.** Real and model-dependent: lfm2.5 bypassed `write_file` for `echo >` and corrupted the content via shell quoting; gemma4 used the right tool and produced valid output. Default switched to `gemma4:latest`. Structural mitigations remain: `max_iterations`, structured tool errors (FR-20), and a system prompt that names the preferred tool for each hazard. |

...
| FR-3a | The transport MUST parse `message.thinking` separately from `message.content`. Reasoning MUST NOT be mistaken for the final answer, and MUST be hidden by default (rendered dimmed only when `think: true`). | Must |
...
| 3 | Ollama's tool-call response shape varies by version (`tool_calls` vs content-embedded JSON), and `thinking` models add a third field. | Claude | Isolate in `_parse_message()`; handle `thinking`; fall back to parsing a JSON object from `content` when `tool_calls` is absent. |
